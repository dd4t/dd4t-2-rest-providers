 ...
 * @since 24/07/14.
 */
public enum TridionCustomMetaQueryProvider {
	...
	public String getComponentsByCustomMeta (String locale, final MultivaluedMap<String, String> queryStringCollection, int templateId) throws ParseException, StorageException, ItemNotFoundException, IOException {
		...
		String key = getKey(CacheType.SearchCustomMeta, publicationId, queryStringCollection);
	...
	public List<ComponentPresentation> getCustomMetaQueryComponentPresentations (final MultivaluedMap<String, String> queryStringCollection, final int templateId, final int publicationId) throws ItemNotFoundException, ParseException, StorageException, UnsupportedEncodingException {
		...
		if (itemUris == null || itemUris.length == 0) {
			...
			return TridionComponentProvider.INSTANCE.getComponentPresentations(itemUris, templateId, publicationId);
	...
	public String getComponentsBySchema (String locale, String schema, int templateId) throws ParseException, StorageException, ItemNotFoundException, IOException {
		...
		final String key = getKey(CacheType.ComponentsBySchema, publicationId, schema, templateId);
		CacheElement<String> cacheElement = cacheProvider.loadFromLocalCache(key);
		String result;
			...
			synchronized (cacheElement) {
				if (cacheElement.isExpired()) {
					...
					if (itemUris == null || itemUris.length == 0) {
						...
						LOG.debug("Found {} results.", itemUris.length);
						final StringBuilder components = new StringBuilder();
						final List<ComponentPresentation> componentPresentations = TridionComponentProvider.INSTANCE.getComponentPresentations(itemUris, templateId, publicationId);
						for (ComponentPresentation componentPresentation : componentPresentations) {
							LOG.debug("Loading {}", componentPresentation.getComponentId());
							components.append(new String(componentPresentation.getContent(), "UTF-8"));
							components.append(DIVIDER);
	...
	public String getComponentsBySchemaInKeyword (String locale, String schema, int categoryId, int keywordId, int templateId) throws ParseException, StorageException, ItemNotFoundException, IOException {
		...
		final String key = getKey(CacheType.ComponentsBySchemaInKeyword, publicationId, schema + "-" + keywordId, templateId);
		CacheElement<String> cacheElement = cacheProvider.loadFromLocalCache(key);
		String result;
			...
			synchronized (cacheElement) {
				if (cacheElement.isExpired()) {
					...
					if (itemUris == null || itemUris.length == 0) {
						...
						List<ComponentPresentation> componentPresentations = TridionComponentProvider.INSTANCE.getComponentPresentations(itemUris, templateId, publicationId);
